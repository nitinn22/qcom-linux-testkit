...
OPTIONS:
  ...
  --test-type <basic|uvc|drc|concurrency|advanced-encode|all>
                        ...
                        - advanced-encode: Advanced encoding tests (downstream only)
                          ...
                          * HEVC UHD Cyclic IR (FR82773)
                          * H.264 VGA Slice MB (FR82771)
                          * HEVC 4K Rotate 90° (FR72846)
...
run_h264_concurrency_test() {
  ...
  # Build pipeline using library function with new 3-parameter signature
  pipeline=$(gstreamer_build_concurrency_decode_pipeline "h264" "$h264_clip" "$detected_stack")
...
run_h265_concurrency_test() {
  ...
  # Build pipeline using library function with new 3-parameter signature
  pipeline=$(gstreamer_build_concurrency_decode_pipeline "h265" "$h265_clip" "$detected_stack")
...
# Test: HEVC_Encode_UHD_Cyclic_IR (FR82773) - Downstream only
run_cyclic_ir_encode_test() {
  testname="HEVC_Encode_UHD_Cyclic_IR"
  log_info "=========================================="
  ...
  # Verify rotation: 90° rotation should swap dimensions (3840x2160 → 2160x3840)
  # Use gst-discoverer to check actual output dimensions
  if command -v gst-discoverer-1.0 >/dev/null 2>&1; then
    log_info "Verifying rotation: checking output dimensions..."
    ...
    log_info "Expected dimensions after 90° rotation: 2160x3840"
    log_info "Actual output dimensions: ${actual_width}x${actual_height}"
    
    # Verify dimensions are swapped (rotated)
    if [ "$actual_width" = "2160" ] && [ "$actual_height" = "3840" ]; then
      log_info "$testname: ✓ Rotation verified (dimensions correctly swapped)"
    else
      log_fail "$testname: FAIL (rotation not applied - expected 2160x3840, got ${actual_width}x${actual_height})"
      fail_count=$((fail_count + 1))
      return 1
    fi
  else
  ...
  # Comprehensive validation using shared helper (with original input dimensions for file checks)
  if gstreamer_validate_encode_output "$testname" "$output_file" "$gstRc" "h265" "3840" "2160" "$duration"; then
    log_pass "$testname: PASS (rotation verified)"
    pass_count=$((pass_count + 1))
    return 0
  else
    fail_count=$((fail_count + 1))
...
run_rotate_encode_test() {
  ...
  # Build pipeline using shared helper function with rotation control
  pipeline=$(gstreamer_build_camera_encode_pipeline \
    "h265" "3840" "2160" "$output_file" \
    "controls,video_bitrate=8000000,rotate=90" \
    "$detected_stack")
  ...
  # Verify rotation using shared helper (90° rotation should swap dimensions: 3840x2160 → 2160x3840)
  if ! gstreamer_verify_rotation "$testname" "$output_file" "3840" "2160" "90"; then
    fail_count=$((fail_count + 1))
    return 1
  fi
