...
**2. HEVC_Encode_UHD_Cyclic_IR (FR82773)**
- **Description**: HEVC UHD (3840x2160) encode with Cyclic Intra Refresh
- **Requirements**: Downstream driver, qtiqmmfsrc, v4l2h265enc
- **Pipeline**: `qtiqmmfsrc → v4l2h265enc (intra_refresh_period_type=1) → h265parse → mp4mux → filesink`
- **Features**: Cyclic intra refresh mode for error resilience
...
**4. HEVC_Encode_4K_Rotate90 (FR72846)**
- **Description**: HEVC 4K (3840x2160) encode with 90° VPU rotation
- **Requirements**: qtiqmmfsrc, v4l2h265enc (rotate control requires downstream driver)
- **Pipeline**: `qtiqmmfsrc → v4l2h265enc (rotate=90) → h265parse → mp4mux → filesink`
- **Features**: Hardware-accelerated rotation during encoding
...
Build pipeline for concurrent decode tests with qtivcomposer (4x4 grid).

- **Parameters**:
  - `codec`: `h264`, `h265`, or `mjpeg`
  - `clip_path`: Path to test clip
  - `sessions`: Number of concurrent sessions (default: 16)
- Returns: Complete pipeline string with 4x4 grid layout
- Automatically handles codec-specific decode chains
- Example:
  ```sh
  pipeline=$(gstreamer_build_concurrency_decode_pipeline \
    "h264" "/tmp/test.mp4" "16")
  gstreamer_run_gstlaunch_timeout 40 "$pipeline"
  ```
...
- Example:
  ...
  pipeline=$(gstreamer_build_camera_encode_pipeline \
    "h265" "3840" "2160" "/tmp/output.mp4" "$controls" "downstream")
  gstreamer_run_gstlaunch_timeout 40 "$pipeline"
  ```
